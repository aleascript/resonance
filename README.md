...
2. La Proposition (Le Changement de Paradigme)
Le Principe de Résonance postule que la mécanique de résolution est l'outil de worldbuilding ultime. Les règles ne doivent pas simplement simuler si une action réussit, elles doivent simuler la manière dont la réalité de cet univers précis réagit à la conscience du personnage. Changer de philosophie intime ou de plan d'existence, c'est changer littéralement de mécanique de jeu.

3. Les Deux Étages du Principe

    Le Signal (L'approche sémantique) : Le monde est bruyant. Le joueur doit y trouver un sens. En s'immergeant dans la diégèse, il extrait des arguments (mots-clés, culture, environnement, émotions) pour former un signal clair et justifié.

    L'Acoustique (La résolution diégétique) : Le monde évalue ce signal selon sa propre vérité. Les dés ne sont plus des générateurs de probabilités aveugles : ils sont la "caisse de résonance" de l'univers, tordant le résultat selon que le monde obéisse à la logique mathématique, à la foi, au code informatique ou au chaos.


# Le Principe de Résonance

## Les Mises et l'Approche Cognitive: le Signal 

### La fin de l'illusion mathématique et du méta-jeu

Depuis des décennies, le jeu de rôle traditionnel s'appuie sur une illusion : celle des pourcentages et des caractéristiques chiffrées. Pourtant, dans la vraie vie, aucun être humain n'aborde un problème en estimant qu'il a "75 % de chances de réussir". Face à un obstacle, un individu évalue instinctivement ses forces, ses faiblesses, son environnement et son état émotionnel. Cette évaluation n'est pas un chiffre froid, c'est un espoir.

S'abriter derrière une feuille de statistiques génère inévitablement du "méta-jeu" : le joueur cesse de regarder le monde à travers les yeux de son PJ pour chercher le "bouton mécanique" le plus optimisé.

Le Principe de Résonance supprime intégralement ces statistiques abstraites. Il propose un système purement diégétique (ancré dans la narration) où la mécanique n'est plus une surcouche qui sort le joueur de l'immersion, mais un prolongement direct de la conscience du personnage.

### Un nouveau simulationnisme : Le modèle "LLM"
...
Pour comprendre cette approche, il faut faire l'analogie avec le fonctionnement d'une intelligence artificielle (les LLM, ou modèles de langage). Une IA ne réfléchit pas avec des règles mathématiques fixes ; elle évalue le "poids sémantique" des mots (les *tokens*) pour générer un contexte (le *prompt*) et prédire la suite logique.

Dans ce système, le PJ et le MJ font exactement la même chose. Face à une situation, chaque camp rassemble des **Mises** : des arguments narratifs justifiés par la fiction. Il peut s'agir d'une arme héritée de ses ancêtres, d'une bourrasque de vent providentielle, d'une haine viscérale, ou d'une discipline de fer. Vous ne faites plus des mathématiques : vous compilez le "code source" de la scène.


### La Mécanique : Focus, Facteurs Cadres et Mises

Lorsqu'un conflit éclate, la résolution passe par l'analyse minutieuse de la situation.
...
### L'effondrement de la fonction d'onde : Le Tirage

Une fois le nombre de Mises défini pour chaque camp, le système bascule dans une logique qui rappelle la **physique quantique**.

Avant le tirage, l'issue de la scène est un nuage de probabilités. La victoire, la défaite, l'exploit ou la catastrophe existent simultanément. Le nombre de Mises rassemblées par le joueur ne garantit pas la réussite ; il ne fait que sculpter et étirer ce nuage de probabilités en sa faveur.

Puis, les dés roulent. C'est le moment de l'observation. À cet instant précis, le lien mental entre le détail narratif (la Mise) et le dé qu'il a apporté disparaît. La fonction d'onde s'effondre pour ne laisser qu'une seule réalité mathématique : le nombre final de réussites. Le chaos des probabilités se fige pour dicter la nouvelle vérité de la fiction, qui sera ensuite interprétée par la table.

C'est la différence de réussites entre les deux camps qui indiquent l'ampleur du succès ou de l'échec.

## Le Calcul Diégétique: l'Acoustique

### Le socle : L'observation de la réalité

Une fois que le joueur a réuni ses Mises sous la forme d'un nombre *n* de dés, il faut les lancer. La question centrale du *game design* intervient à cet instant précis : **comment l'univers détermine-t-il le nombre de réussites à partir de ces dés ?**

Dans un monde strictement physique et dénué de magie (le mode "agnostique" ou "médian"), la règle par défaut est d'une simplicité binaire : **un résultat pair (2, 4, 6) génère 1 réussite, un résultat impair (1, 3, 5) est un échec**. C'est la réalité fondamentale, froide et indifférente.

Cependant, se contenter de ce socle serait passer à côté de l'essence même du Principe de Résonance. Le véritable défi du *game designer* commence ici : il s'agit d'altérer cette règle mathématique pour qu'elle devienne le miroir parfait de la réalité (ou de la philosophie) dans laquelle il veut immerger ses joueurs. La façon de lire les dés *est* le *worldbuilding*.

### Proof of Concept : Les Cadres de Pensée dans Glorantha Perspectives

Pour illustrer ce concept, le jeu [Glorantha Perspectives](https://aleascript.github.io/glorantha-perspectives/content/fr/srd/glorantha-perspectives-fr.pdf) propose une démonstration magistrale. Dans ce monde, la magie n'est pas une liste de sorts, c'est une manière d'appréhender l'univers. La mécanique mute donc selon la philosophie de celui qui lance les dés :
...
### L'horizon du Game Designer : Concevoir ses propres acoustiques
...
Voici comment le Principe de Résonance peut être étendu et hacké :
...
L'acoustique peut dépendre de la philosophie du personnage, mais aussi du *plan d'existence* dans lequel il évolue.
...
### Conclusion : La règle est le monde

Le Principe de Résonance replace le créateur de jeu dans son véritable rôle : celui d'un architecte de réalités. En modelant la façon dont les dés réagissent aux Mises narratives, le *game designer* ne se contente plus de fournir un arbitre neutre à sa table ; il offre aux joueurs la possibilité de toucher, de manipuler et de ressentir physiquement les lois métaphysiques de l'univers et de s'immerger totalement dans la fiction sans recourir à du meta-jeu qui les en éloigne.

Voici une proposition pour ce paragraphe, pensé pour s'insérer parfaitement à la fin de la **Partie 1** de votre manifeste. Il vient appuyer votre démarche d'accessibilité et faire tomber le cliché du "JDR tableur".


### L'évidence pour les débutants, un défi pour les vétérans

Longtemps, le jeu de rôle a renvoyé au grand public l'image d'un loisir réservé à des initiés jonglant avec des calculs abscons, masquant ainsi la véritable richesse créative et narrative de ce médium. Et quel joueur n'a pas été sorti de l'immersion par une procédure de jeu complexe, ou nécessitant de chercher dans le manuel la référence exacte? Le *Principe de Résonance* a donc été aussi conçu pour briser cette barrière. Je pense même que ce sont les joueurs totalement débutants qui se sentiront le plus à l'aise et désinhibés face à ce système. La raison en est simple : la création des Mises reproduit avec exactitude le fonctionnement cognitif universel de n'importe quel être humain. Dans la vraie vie, personne ne calcule un pourcentage de réussite ; par contre, tout le monde évalue de manière naturelle et instinctive ses forces, ses faiblesses ou son environnement sans avoir besoin de les quantifier. Un néophyte décrira spontanément en quoi la pluie battante ou son courage l'aident à surmonter l'obstacle. À l'inverse, ce sont les vétérans du jeu de rôle qui peuvent se retrouver momentanément déroutés : pour maîtriser ce système, ils doivent désapprendre des décennies de réflexes mécaniques, abandonner la recherche d'un bonus chiffré sur leur feuille, et réapprendre tout simplement à regarder le monde.
...
"La contimanation vous guette" : risque de transformation irréversible ou de souillure permanente.
